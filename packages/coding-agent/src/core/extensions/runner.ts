...
import type { ImageContent, Model, Provider, ProviderHeaders } from "@liyuan/ai";
...
export class ExtensionRunner {
	...
	async emitMessageEnd(event: MessageEndEvent): Promise<MessageEndEventResult | undefined> {
		...
		for (const ext of this.extensions) {
			...
			for (const handler of handlers) {
				try {
					...
					if (handlerResult?.persist === false) transient = true;
		...
		return modified || transient
			? { ...(modified ? { message: currentMessage } : {}), ...(transient ? { persist: false as const } : {}) }
			: undefined;
	}
