...
function modelFromJson(
	...
	let compat = mergeCompat(providerConfig.compat, definition.compat);
	if (api === "openai-completions" && !(compat && "maxTokensField" in compat && compat.maxTokensField)) {
		const official = baseUrl.includes("api.openai.com") || baseUrl.includes("openai.azure.com") || baseUrl.includes("cognitiveservices.azure.com");
		if (!official) compat = { ...compat, maxTokensField: "max_tokens" };
	}
	return {
		id: definition.id,
		...
		provider: providerId,
		baseUrl,
		// A configured thinking level, including off, needs reasoning controls to be sent.
		reasoning: definition.reasoning ?? definition.thinkingLevel !== undefined,
...
function applyModelsJson(
	...
	// Liyuan channels with their own endpoint and catalog replace the built-in catalog.
	const selectedBase = config.baseUrl && config.models?.length ? [] : baseModels;
	const models: Model<Api>[] = selectedBase.map((model) => ({
		...model,
	...
	for (const definition of config.models ?? []) {
		...
		const defaults = existingIndex >= 0 ? models[existingIndex] : (baseModels.find((model) => model.id === definition.id) ?? baseModels[0]);
