    ...
    (async () => {
        ...
        try {
            ...
            if (compat.streaming !== false) {
                const { data: openaiStream, response } = await retryProviderRequest(() => client.chat.completions.create(params, requestOptions).withResponse(), retryOptions);
                ...
                const { data: completion, response } = await retryProviderRequest(() => client.chat.completions.create(nonStreamingParams, requestOptions).withResponse(), retryOptions);
                await options?.onResponse?.({ status: response.status, headers: headersToRecord(response.headers) }, model);
...
function detectCompat(model) {
    ...
    const isDeepSeek = provider === "deepseek" || baseUrl.toLowerCase().includes("deepseek.com") || /deepseek/i.test(model.id);
