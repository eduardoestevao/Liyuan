		...
		try {
			...
			if (model.compat?.streaming !== false) {
				const { data: openaiStream, response } = await retryProviderRequest(
					() => client.responses.create(params, requestOptions).withResponse(), retryOptions,
				);
				await options?.onResponse?.({ status: response.status, headers: headersToRecord(response.headers) }, model);
				eventSource = openaiStream;
			} else {
				const { data: fullResponse, response } = await retryProviderRequest(
					() => client.responses.create({ ...params, stream: false }, requestOptions).withResponse(), retryOptions,
				);
				await options?.onResponse?.({ status: response.status, headers: headersToRecord(response.headers) }, model);
