...
```ts
import { PiClient, type ByteTransportFactory } from "@liyuan/client";
...
```ts
import { PiClient } from "@liyuan/client";
import { createUnixTransportFactory } from "@liyuan/client/unix";
...
The `@liyuan/client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@liyuan/client/unix` subpath.
