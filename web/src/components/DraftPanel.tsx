...
const phases = { exploring: "探索中", writing: "写作中", waiting: "等待回答", sealed: "已收笔", stopped: "已停止，稿件已保存", error: "发生错误，稿件已保存" };
...
export function DraftPanel({ workspace, revisions, busy, onInspect, onRestore }: {
	...
	return <details className="draft-panel" onToggle={(e) => { if (e.currentTarget.open) onInspect(); }}>
		<summary>{workspace.revision ? "上一拍修订" : "本拍稿件"} · {phases[workspace.phase]} · v{workspace.version}</summary>
		...
		{workspace.revisions.length > 0 && <>
			<label>查看版本 <select value={selected} onChange={(e) => { setSelected(Number(e.target.value)); onInspect(); }}>
				{workspace.revisions.map((r) => <option key={r.version} value={r.version}>v{r.version} · {new Date(r.at).toLocaleTimeString()}</option>)}
			</select></label>
			<textarea aria-label="稿件版本原文" readOnly value={text ?? "正在读取…"} rows={8} />
			<button type="button" disabled={text === undefined || selected === workspace.version || (busy && workspace.phase !== "waiting")} onClick={() => onRestore(selected)}>恢复此版本</button>
			<small>恢复会新增一个文本版本；世界线与账本不回退。</small>
		</>}
			{pendingText && <label>未完成稿件（尚未形成新版本）<textarea aria-label="未完成稿件" readOnly value={pendingText} rows={8} /></label>}
		{workspace.context && <details><summary>上下文用量</summary><small>{workspace.context.messages} 条消息，{workspace.context.chars.toLocaleString()} 个字符；本次移除了 {workspace.context.prunedResults} 份重复或过期的工具结果正文，共 {workspace.context.prunedChars.toLocaleString()} 个字符。此数为字符量，不是计费 token。</small></details>}
	</details>;
}
