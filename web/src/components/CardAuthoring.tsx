...
export function CardAuthoring({ card, onApplied }: { card: string; onApplied: () => void }) {
	...
	const check = () => run(async () => {
		...
		setBuild(result);
		await refresh();
		if (!result.errors.length) setNotice("资源检查通过，" + result.changed.length + " 项变更");
	});
	const showPreview = () => run(async () => {
		...
		if (!values || typeof values !== "object" || Array.isArray(values)) throw new Error("测试变量需为 JSON 对象");
		...
		<section className="sp-section card-authoring">
			...
			{open && <>
				...
				{status && !status.prepared && <div className="panel-row">
					<span>{status.resources.length} 项资源</span>
					<button className="drawer-btn" disabled={busy} onClick={prepare}>建立创作稿</button>
				</div>}
				{status?.prepared && <>
					<div className="panel-row card-authoring-toolbar">
						<span>{status.resources.filter(r => r.changed).length} 项待应用</span>
						...
						{status.canUndo && <button className="act" disabled={busy || dirty || status.conflict} onClick={() => void run(async () => {
								setStatus(await operation<CardProjectStatus>({ action: "undo" })); setBuild(null); setPreview(null); onApplied(); setNotice("已撤回应用，创作稿仍保留");
						})}>撤回应用</button>}
					</div>
					{status.conflict && <p className="card-authoring-error" role="status">原卡已有其他修改，创作稿已保留，应用将停止覆盖。</p>}
											<label className="card-authoring-label">
							资源
							<select aria-label="卡资源" value={selected} disabled={busy || dirty} onChange={e => { setSelected(e.target.value); setBuild(null); }}>
								<option value="">选择资源</option>
								{status.resources.map((r, i) => <option key={r.id} value={r.id}>{i + 1}. {r.name}{r.changed ? " · 已修改" : ""}</option>)}
								<option value="raw">原包 JSON（只读）</option>
							</select>
						</label>
					{source && <>
						<textarea className="panel-search ta card-source-editor" aria-label="资源源码" spellCheck={false}
								readOnly={selected === "raw"} disabled={busy} rows={12} value={text} onChange={e => { setText(e.target.value); setBuild(null); setPreview(null); }} />
						<div className="panel-row card-authoring-toolbar">
							<button className="drawer-btn" disabled={busy || !dirty || selected === "raw"} onClick={() => void run(async () => { await save(); setNotice("创作稿已保存"); })}>保存创作稿</button>
							{dirty && <button className="act" disabled={busy} onClick={() => setText(source.text)}>还原编辑</button>}
						</div>
					</>}
					<details className="legacy-group">
						<summary>测试内容与变量</summary>
						<label className="card-authoring-label">测试消息
							<textarea className="panel-search ta" aria-label="测试消息" rows={5} value={message ?? ""} placeholder="留空使用默认开场"
								onChange={e => setMessage(e.target.value)} />
						</label>
						<label className="card-authoring-label">测试变量
							<textarea className="panel-search ta card-source-editor" aria-label="测试变量" rows={5} value={variables ?? ""}
								placeholder="首次预览读取卡内初值" onChange={e => setVariables(e.target.value)} />
						</label>
					</details>
					<div className="panel-row card-authoring-toolbar">
						<button className="drawer-btn" disabled={busy} onClick={check}>检查</button>
						<button className="drawer-btn" disabled={busy} onClick={showPreview}>预览</button>
						<button className="drawer-btn save-btn" disabled={busy || dirty || !build || !!build.errors.length || !build.changed.length || status.conflict}
							onClick={() => void run(async () => {
								...
								setBuild(null); onApplied(); setNotice("已应用并重载");
							})}>应用到角色卡</button>
					</div>
					...
					{preview && <div className="card-preview">
						<div className="panel-row card-authoring-toolbar"><span>测试预览</span><button className="act" onClick={() => setPreview(null)}>关闭预览</button></div>
						<iframe key={preview.token} ref={frame} title="角色卡创作预览" sandbox={CARD_PREVIEW_SANDBOX} src={preview.url} />
