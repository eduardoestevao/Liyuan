...
| # | 通道 | 今天位置 | 目标 | 理由 |
|---|---|---|---|---|
| A1 | **梨园身份 / 一拍是什么 / 稿纸纪律 / 检索纪律** | **不存在** | **默认 APPEND_SYSTEM.md（用户可改，§七G 修订：不进 SYSTEM.md——那是环境底座）** | 这就是「agent 很傻只会思考输出」的空缺。60 个工具全靠 pi 原生工具描述自我介绍，没有一个字讲流程 |
...
## 七D、刀4 执行记录（2026-09-08）

**实测定形**：25 本书 1907 条启用里协议条目共 24 条（21 蓝 / 3 绿，集中 8 本 MVU 系书；
绿灯仅凡人修仙传 3 条、其中 1 条疑似误判——留给用户在判定文件里改判）。

**运行时正则退场**（`src/lorebook-declare.ts`）：
- 装配只执行**判定数据**——每本书旁一份 `<书名>.json.判定.json`（uid/标题/字数/家族/信号，人可读可改可删）；
- 没有判定文件的书**不过滤**：没有数据就没有动作，不做隐形兜底（手工拷文件的 用户自己补一次判定，书单可见状态）；
- detectProtocol 降级为**数据生产工具**，只在两个时机跑：导入时（lorebooks/import 自动）与手动（POST /api/lorebooks/declare，幂等，判 0 条即删判定回到不过滤）；
- 按书应用（uid 是书内编号，跨书合并后套会误杀同号）；判定文件进 materials 指纹（改判定下一拍生效）；
- stripMvuRuleEntries 保留——判据是**归属**（场记的变量规则）不是作者措辞，铁律三不禁。
- 存量 25 本书已回填判定（8 本 24 条，与原运行时行为逐条一致），从隐形正则变成 8 份可见数据。

**书单 GET /api/lorebooks 每本新增 `declared`**（null=无判定文件，数字=判定停用条数）——
前端把判定状态露出来（等用户回来一起改前端，见 §九）。

## 七E、刀5 处置记录（2026-09-08）

原验收「模型自己查得到，『查过了』有依据」**已由刀1/刀3 达成**，三拍实测：
- 刀1 拍（无档案）：工具 0→6，lorebook_search×2 真查（冷鹰命中 3 条、烂柯寺无命中——「没有」第一次有真实依据）；
- 刀3 拍（有档案）：先查世界状态再定好感度数值，6 工具。
失败机制（标题列表被当全集）在 SYSTEM.md「写之前先查」一节已反写（索引有字数上限、lorebook_list 看全量），无数据支持再改索引形状。

**世界书文件化**（B6/B7 的彻底形态：书变成卡目录里的文件、与 AGENTS.md 同构）**移入第二步「角色卡制作」**——它是卡制作的地基（世界书可编辑可版本化），不是扮演体验的紧急修复；与前端改造一起等用户回来定。

## 九、待用户回来定的前端清单（2026-09-08 暂存）

1. 世界书面板：书单露 `declared` 状态（null/数字）＋「检查一遍」（POST /api/lorebooks/declare）按钮；
2. 「这张卡」页签的生成按钮入口更醒目／首次使用引导（22 张存量卡的档案迁移靠它）；
...
## 七G、SYSTEM.md 越权修正（2026-09-08，用户定性后重构）

**用户点破的矛盾**：站在全局的 agent 与扮演沉浸性之间，梨园替用户选了沉浸——「你在
梨园里担任角色扮演 agent」写在 SYSTEM.md 里，等于把「发『你好』是自我介绍还是入戏」
这个选择从用户手里抢走。这和 protocol-detect 是同一个病：harness 替用户做了本该用户
做的决定。「不要替用户决定他的角色说什么做什么」这句本身也在规定怎么扮演——越权。

**对照原版**（pi-mono main 与 vendored 0.84.4 逐字一致）：pi 默认提示词＝一句身份
（You are an expert coding assistant…）＋工具表＋两条 guideline，零行为规定；
`SYSTEM.md` 的文档语义＝**替换默认**，`APPEND_SYSTEM.md`＝**追加**。

**重构**：
- `assets/SYSTEM.md` → 196 字最小环境底座（一句身份＋工作区事实），不规定怎么做事；
- `assets/APPEND_SYSTEM.md`（新随包件）→ 扮演定义的**默认值**（一拍/稿纸/检索/岔口/
  记账——原 SYSTEM.md 全文平移，含主权澄清句——它是可删改的默认，不是 harness 法）；
- 播种两份（缺失才播）；**一次性迁移**：现存 SYSTEM.md 以旧开场句开头（梨园自己发的
  旧底座）⇒ 全文挪去 APPEND（若 APPEND 尚不存在）、底座换新；用户改写过的（签名不符）
  一字不动；
- 想要全局 agent 式应答（「你好，我是梨园，我能……」）的用户改 APPEND_SYSTEM.md 即得，
  想要更严的沉浸也改它——**矛盾变成槽位里的一个选择**。

§二映射表 A1 行相应修订：身份/一拍/稿纸/检索纪律 → **默认 APPEND_SYSTEM.md（用户可改）**，
不再是 SYSTEM.md。

## 七H、提示词条目：引擎＋编辑器（2026-09-08，用户点出文件化丢掉了开关的易用性）

**用户的例子**：预设切人称＝关掉第一人称条目、打开第二人称；.md 文件却要翻到
人称那段去改字。酒馆的易用性核心是「条目＋开关」，文件化不能把它丢掉。

**形状（src/prompt-entries.ts，纯函数，前后端共用）**：
- `## 小节` ＝ 条目（名字＝标题，正文原样）；标题层级 1~6 都算（卡档案的 `###`
  状态栏子节＝正好是逐个开关的粒度）
- **整段包在 HTML 注释里 ＝ 关闭**（`<!--` 独占一行开、`-->` 独占一行闭、内部首行
  是标题）——markdown 通用的「注释掉」，GitHub 渲染自然隐藏，零新语法
- 其余注释 ＝ 人看的备注：送模剥掉、条目视图不管（活在原文里）

**引擎**：`renderForModel(md)`——解析后只留开启条目、剥全部注释。materials 对
两级 APPEND 与卡 AGENTS.md 的**送模面**都走它；`readUserRules`/REST 继续给原文
（编辑器往返无损——渲染只发生在送模那一刻）。全关 ⇒ 送模空串（用户的明确选择），
「档案在场」判据仍看原文。

**编辑器**（前端「提示词」面板，三个条目形文件：全局 APPEND／卡级 APPEND／
AGENTS.md）：GitHub 式双视图——**条目**（卡片：名字/字数/开关，展开改正文、
删除；底部添加条目）与**源码**（raw markdown）。条目视图的修改是对原文的
**定点手术**（toggle/setContent/append/delete，不重排不丢备注）；SYSTEM.md 是
底座 blob，保持纯源码编辑器。

## 七I、收窗（2026-09-08，用户拍板「提示词板块完成」）

本路线全部落地（八提交见 PLAN-RP-AGENT-NEXT.md §一）。提示词板块终态：
「提示词」面板＝全局系统提示词（SYSTEM.md 底座＋APPEND 全局）＋局部提示词
（AGENTS.md 卡档案＋APPEND 卡级）＋预设库（转译素材），条目引擎＋双视图编辑器，
观感规则已立。**下一步＝第二步角色卡制作工具，调查以代码维度为中心**
（用户点名：卡背后有代码）——见 PLAN-RP-AGENT-NEXT.md §一A。
