...
- **JSON 卡封面＝侧挂文件**（用户点名「json 没带封面图片的卡无法更换封面」）：`setCardCover` 不再拒 JSON 卡；
  应用时卡本体仍是纯 JSON，封面字节落**同目录同名 `.png` 侧挂**（寻址唯一主人 `src/card.ts coverSidecarOf`，
  工坊应用／图片端点回落／导出 PNG 当壳／删卡清理四处共用）。裸图不含卡数据，卡库扫描按「解析不出卡名」
  自动跳过；卡文件夹内 `cardFileIn` 字典序 `.json` 先于 `.png`，卡本体身份不受影响。撤回按应用前的侧挂
  存档整张恢复（`undo-cover.json/png`）。库列表加 `hasCover`，前端有侧挂的 JSON 卡显示封面而非「JSON」徽标；
  导出 PNG 用侧挂当壳替掉占位图。测试：card-authoring 侧挂全链路（应用/撤回两轮/导出）、card-changes 旧
  「JSON 卡拒绝」断言改受理；全量 801/797（4 红既有实卡基线）。
- **01 板块改名**（用户定性「角色设定窄化了——角色卡更多时候是一个世界」）：「角色设定」→「世界与角色设定」，
  描述与字段提示语去主角中心化（核心设定＝世界观/规则系统或角色性情；世界与处境＝舞台格局）。只动
  CardStudio 展示层，卡字段（personality/scenario 等 ST 标准槽位）不动。
- 工坊与扮演的**世界书同步问题**经查证后用户定案**不做**：工坊 02 编辑卡内嵌 character_book（随卡走），
  扮演只吃全局库挂载＋补充设定集（v1.5 解耦定案），「另存为独立世界书」是 fork 不是链接——现状即设计。
